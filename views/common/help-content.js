/**
 * Help / Usage content shared between the in-page help panel
 * (views/main/actions/help.js) and the toolbar popup (views/popup/).
 *
 * Returns the inner `<ul class="main-list">` markup. References the global
 * `icons` (available in both the content script and the popup). `isMac` is
 * a local const here so it does not collide with the global `isMac` declared
 * in views/main/actions.js when both load in the same content-script scope.
 *
 * @returns {string}
 */
